...
// ===============================================
app.get('/', (req, res) => { res.status(200).send('✅ Servidor AUTOLOG estable y seguro.'); });

// ===============================================
// 3. CONEXIÓN Y CIRUGÍA DE BASE DE DATOS (NEON)
// ===============================================
let pool;
try {
    ...
    const inicializarEstructura = async () => {
        const consultas = [
            ...
            // CIRUGÍA: Actualizar tablas antiguas de forma segura
        ...
        for (let sql of consultas) {
            ...
            catch(e) { console.log(`Aviso en tabla: ${e.message}`); }
        }
    };
...
// ===============================================
// VEHÍCULOS (BLINDADO CONTRA CAÍDAS)
// ===============================================
app.get('/api/vehiculos', async (req, res) => { 
    try { 
        const r = await pool.query('SELECT * FROM vehiculos ORDER BY marca_auto, modelo'); 
        res.json(r.rows); 
    } catch (err) { res.status(500).json({ error: err.message }); } 
});

app.post('/api/vehiculos', async (req, res) => { 
    const { marca_auto, modelo } = req.body; 
    try { 
        await pool.query('INSERT INTO vehiculos (marca_auto, modelo) VALUES ($1, $2)', [marca_auto, modelo]); 
        res.json({ success: true }); 
    } catch (err) { 
        console.error("Error al guardar auto:", err.message);
        res.status(500).json({ error: "Fallo estructural en tabla Vehiculos: " + err.message }); 
    } 
});

app.delete('/api/vehiculos/:id', async (req, res) => { 
    try { 
        await pool.query('DELETE FROM aplicaciones WHERE vehiculo_id = $1', [req.params.id]); 
