...
app.get('/api/productos', async (req, res) => { 
    try { 
        const query = `
            ...
            COALESCE((SELECT json_agg(v.id) FROM aplicaciones a JOIN vehiculos v ON a.vehiculo_id = v.id WHERE a.producto_id = p.id), '[]') as vehiculos_ids,
            COALESCE((SELECT string_agg(v.marca_auto || ' ' || v.modelo, ', ') FROM aplicaciones a JOIN vehiculos v ON a.vehiculo_id = v.id WHERE a.producto_id = p.id), '') as vehiculos_nombres
            FROM productos p 
...
app.post('/api/productos', upload.single('imagen'), async (req, res) => { 
    const { codigo_pieza, marca_id, categoria_id, descripcion, precio, stock, vehiculos_compatibles } = req.body; 
    const imagen_url = req.file && req.file.path ? req.file.path : null; 
    let vehiculos = []; try { vehiculos = JSON.parse(vehiculos_compatibles || '[]'); } catch(e){}
    try { 
...
app.put('/api/productos/:id', upload.single('imagen'), async (req, res) => { 
    const { id } = req.params; 
    const { codigo_pieza, marca_id, categoria_id, descripcion, imagen_url_actual, precio, stock, vehiculos_compatibles } = req.body; 
    let imagen_url = req.file && req.file.path ? req.file.path : imagen_url_actual; 
    let vehiculos = []; try { vehiculos = JSON.parse(vehiculos_compatibles || '[]'); } catch(e){}
    try { 
        await pool.query('BEGIN');
...
app.post('/api/ventas', upload.single('comprobante'), async (req, res) => {
    ...
    try {
        await pool.query('BEGIN');
        const resVenta = await pool.query("INSERT INTO ventas (metodo_pago, total, comprobante_url, estado) VALUES ($1, $2, $3, 'Pendiente') RETURNING id", [metodo_pago, total, comprobante_url]);
