...
import Login from './Login'; // AQUÍ CONECTAMOS LA SEGURIDAD

const URL_BACKEND = 'https://autolog-catalogo.onrender.com'; 
const WHATSAPP_VENDEDOR = "584124172323"; 
...
function App() {
  ...
  const agregarAlCarrito = (producto) => {
    ...
    setCarrito(prev => {
      ...
      if (existe) {
        if (existe.cantidad >= producto.stock) { alert(`Solo quedan ${producto.stock} uds.`); return prev; }
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex gap-4 items-center">
            {vista === 'admin' && autenticado && (
              <button onClick={() => { setAutenticado(false); localStorage.removeItem('auth_autolog'); setVista('catalogo'); }} className="text-red-500 font-bold hover:bg-red-50 px-3 py-1.5 rounded-lg transition">Cerrar Sesión</button>
            )}
      ...
      {vista === 'catalogo' ? (
        <main className="flex-1 max-w-7xl mx-auto w-full p-6 flex flex-col lg:flex-row gap-8">
          <div className="flex-1">
            <input type="text" placeholder="Buscar por código o pieza..." className="w-full p-4 rounded-xl border-2 border-slate-200 bg-white shadow-sm outline-none focus:border-blue-500 mb-6 font-bold" value={busqueda} onChange={e => setBusqueda(e.target.value)} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {productos.filter(p => ((p.codigo_pieza || '').toLowerCase().includes(busqueda.toLowerCase()) || (p.descripcion || '').toLowerCase().includes(busqueda.toLowerCase())) && (filtroMarca === 'Todas' || p.marca === filtroMarca)).map(p => (
                <div key={p.id} className="bg-white p-6 rounded-2xl border-2 border-slate-200 shadow-sm flex flex-col hover:border-blue-300 transition">
