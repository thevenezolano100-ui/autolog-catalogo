...
function Admin() {
  ...
  // CONTROLADORES DE AJUSTES GENERALES (CON LECTOR DE ERRORES REAL)
  const accionSimple = async (ruta, metodo, cuerpo) => { 
    ...
    try {
        ...
        setMensaje('⏳ Enviando instrucción...');
        ...
        if(!res.ok) { 
            try { 
                ...
                alert(`❌ Servidor indica: ${err.error}`); 
            } catch { 
                alert(`❌ Servidor Caído: ${respuesta.substring(0, 100)}...`); 
            } 
            return; 
        }
        
        alert('✅ ¡Proceso completado de forma exitosa!'); 
        setNuevaMarca(''); setNuevaCategoria(''); setNuevoVehiculo({marca_auto:'', modelo:''}); 
        cargarTodo(); 
    } catch(e) { 
        setMensaje(''); 
        alert(`❌ Falla de red: Servidor no disponible.`); 
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-6">
      ...
      <div className="flex flex-wrap gap-2 mb-8 border-b-2 border-slate-200 pb-4">
        ...
        <button onClick={()=>setTabActiva('AUDITORIA')} className={`font-black px-4 py-2 rounded-xl transition-all ${tabActiva === 'AUDITORIA' ? 'bg-emerald-600 text-white shadow-md' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}>💰 Auditoría</button>
                ...
                <tbody className="divide-y-2 divide-slate-100">
                    {inventario.map(p => (
                        <tr key={p.id} className="hover:bg-slate-50">
                            ...
                            <td className="p-4"><p className="text-xs font-bold text-indigo-600 bg-indigo-50 p-1.5 rounded inline-block truncate max-w-[200px]">{p.vehiculos_nombres || 'Universal'}</p></td>
      ...
      {tabActiva === 'VEHICULOS' && (
        <div className="bg-white p-6 rounded-2xl border-2 border-slate-200 shadow-sm">
            <h3 className="font-black text-slate-900 text-lg mb-6">🚗 Catálogo de Modelos</h3>
      ...
      {tabActiva === 'AUDITORIA' && (
        <div className="bg-white rounded-2xl border-2 border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6 bg-slate-900 text-white"><h2 className="text-xl font-black">💰 Auditoría de Ventas</h2></div>
            <div className="overflow-x-auto p-6">
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y-2 divide-slate-100">
                        {ventasList.length === 0 && (
                            <tr><td colSpan="6" className="p-10 text-center font-bold text-slate-400 text-lg">Aún no hay ventas registradas en el sistema.</td></tr>
                        )}
                        {ventasList.map(v => (
