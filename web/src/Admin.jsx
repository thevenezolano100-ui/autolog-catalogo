...
function Admin() {
  ...
  // ==========================================
  // FUNCIÓN BLINDADA PARA GUARDAR VEHÍCULOS
  // ==========================================
  const guardarVehiculo = async () => {
    if (!isOnline) return alert("⚠️ Necesitas conexión a internet.");
    if (!nuevoVehiculo.marca_auto || !nuevoVehiculo.modelo) return alert('⚠️ Por favor llena la marca y el modelo del vehículo.');
    
    try {
        setMensaje('⏳ Registrando vehículo en la base de datos...');
        ...
        // TRADUCTOR DE ERRORES SEGURO (Evita el falso Error de Red)
        ...
        setMensaje('');
        alert(`❌ Falla crítica de Red: El servidor Render no responde. (${e.message})`);
    }
  };

  // ==========================================
  // OTRAS FUNCIONES DEL ERP (RESTAURADAS AL 100%)
  // ==========================================
