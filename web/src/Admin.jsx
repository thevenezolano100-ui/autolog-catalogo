...
// ENLACE CENTRALIZADO DE TU SERVIDOR EN RENDER
const URL_BACKEND = 'https://autolog-catalogo.onrender.com';

function Admin() {
  // ESTADOS DEL REPUESTO
  const [formulario, setFormulario] = useState({ id: null, codigo_pieza: '', descripcion: '', marca_id: '', categoria_id: '', imagen_url_actual: '', vehiculos_compatibles: [], precio: '' });
  ...
  // --- MOTOR DE CARGA MASIVA DE EXCEL ---
  const procesarExcel = (e) => {
    const archivo = e.target.files[0];
    if (!archivo) return;
    
    if (!window.confirm("¿Estás seguro de subir este inventario masivo? Asegúrate de que las columnas se llamen: codigo, descripcion, marca, categoria, precio.")) {
        document.getElementById('inputExcel').value = '';
        return;
    }

    setCargandoExcel(true);
    setMensaje('⚙️ Procesando archivo Excel... por favor no cierres la pestaña.');
    ...
    reader.onload = async (evento) => {
      try {
        ...
        for (let fila of filas) {
          ...
          formData.append('precio', fila.precio || 0); // Manejo del precio desde Excel
          formData.append('vehiculos_compatibles', '[]');

          const res = await fetch(`${URL_BACKEND}/api/productos`, { method: 'POST', body: formData });
          if(res.ok) exitos++;
        }
        setMensaje(`✅ Carga masiva completada: ${exitos} repuestos añadidos al inventario.`);
        cargarDatosBasicos();
      } catch (error) {
        setMensaje('❌ Error crítico al leer el archivo Excel.');
      } finally {
  ...
  const guardarRepuesto = async (e) => {
    ...
    datos.append('precio', formulario.precio || 0); // Guardado manual del precio
  ...
  const iniciarEdicionRepuesto = (p) => { setFormulario({ id: p.id, codigo_pieza: p.codigo_pieza, descripcion: p.descripcion, marca_id: p.marca_id, categoria_id: p.categoria_id, precio: p.precio || '', imagen_url_actual: p.imagen_url, vehiculos_compatibles: p.vehiculos_compatibles || [] }); setRepuestoEditando(true); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const cancelarEdicion = () => { setFormulario({ id: null, codigo_pieza: '', descripcion: '', marca_id: marcasLista[0]?.id, categoria_id: categoriasLista[0]?.id, precio: '', imagen_url_actual: '', vehiculos_compatibles: [] }); setRepuestoEditando(false); setImagenArchivo(null); document.getElementById('inputImagen').value = ''; };
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-6 space-y-8">
      ...
      {/* MODULO NUEVO: CARGA MASIVA DE EXCEL */}
      <div className="bg-white rounded-2xl shadow-sm border border-emerald-200 overflow-hidden relative">
        <div className="absolute top-0 right-0 p-4 opacity-10 text-6xl">📊</div>
        <div className="p-8">
            <h2 className="text-2xl font-black text-slate-800 mb-2 flex items-center gap-2"><span className="text-emerald-500">⚡</span> Carga Masiva (Excel)</h2>
            <p className="text-sm text-slate-600 mb-6 max-w-2xl">Ahorra tiempo subiendo tu inventario de golpe. Crea un archivo <span className="font-bold text-emerald-600">.xlsx</span> en Excel que contenga exactamente estas cinco columnas en la primera fila: <b>codigo</b>, <b>descripcion</b>, <b>marca</b>, <b>categoria</b>, <b>precio</b>.</p>
            <div className="flex flex-col md:flex-row items-center gap-4">
                <input id="inputExcel" type="file" accept=".xlsx, .xls" onChange={procesarExcel} disabled={cargandoExcel} className="block w-full md:w-auto text-sm text-slate-500 file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:text-sm file:font-black file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer transition-colors shadow-sm"/>
                {cargandoExcel && <span className="text-sm font-bold text-emerald-600 animate-pulse bg-emerald-50 px-4 py-2 rounded-lg border border-emerald-100">Leyendo filas...</span>}
            </div>
        </div>
      </div>
